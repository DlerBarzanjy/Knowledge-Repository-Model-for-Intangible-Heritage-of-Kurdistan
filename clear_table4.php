
<HTML> 
<HEAD>
<META HTTP-EQUIV=REFRESH CONTENT="0;URL=manage_user.php">
<meta http-equiv="Content-Type" content="text/html; charset=utf-8">
<style type="text/css">
body {
	background-image: url(../../final26/Web/images/_gen_img2_1020.jpg);
}
body {
	color: #FFF;
}
</style>
</HEAD>
<div align="center"></div>

</HTML>
<?php

/*
*	Before this script is executed make sure that you have the proper settings in 
*	sql_data.php.
*/

	include "sql_comm_helper4.php";	
	
	if ($_POST['delete_this'])
	{
		login_mysql();
		select_db(true);
		clear_login_table();
		echo ("If no errors reported above the uploaded files are deleted!");
	}
	else if ($_POST['delete_checkboxes_button'])
	{
		login_mysql();
		select_db(true);	
		// Delete entry.
		if ($_POST['id_delete'])
		{		
			foreach ($_POST['id_delete'] as $cb)
			{
				echo "Deleting entry $cb<br>";
				delete_id_from_login_table($cb);
			}
		}
		// Delete file only.
		if ($_POST['id_delete_login_only'])
		{		
			foreach ($_POST['id_delete_login_only'] as $cb)
			{
				echo "Deleting file $cb<br>";
				delete_id_file_only_from_login_table($cb);
			}
		}
	}	
	else
	{
		echo ("ERROR! This file will only be executed if called from manage_user.php!");
	}
?>